...
页码 | 题目 |  举例 | 实现思路（复杂度） 
------|-------|--------|-------
51    | 5.将字符串中的空格替换为“%20”（要求在原字符串上进行替换）| “I am” 变为“I%20am”   | 先计算整个字符串有多少个空格，然后**从后往前**，扫描整个字符串，依次将各个字符移到后面的指定位置，如果遇到空格就替换成 %20
124   | 19.正则表达式匹配：“.”代表任意一个字符，“*”代表它前面的字符可以出现任意次 | 字符串“aaa” 与模式“a.a”和“ac*a”相匹配 | 每次从字符串中拿出一个字符跟模式中的字符去匹配，（1）先匹配当前位的下一位字符是不是“*”；（2）如果下一位字符是 “*”，则判断当前位是否匹配；（2.1）如果当前位匹配（字符相同或者模式字符为“.”），则分别按照“‘*’匹配前面的0个、1个、多个字符的模式来进位递归匹配；（2.2）如果当前位不匹配，按照“‘*’匹配前面的0个字符的模式来进位；（3）如果下一位字符不是 “*”，则直接判断当前位是否匹配，然后再进位递归匹配。
127   |  20.表示数字的字符串：表示数值的字符串遵循模式 `A[.[B]][e/EC]` 或者 `.B[e/EC]`，且整数位前可能有符号位 | “+1000”、“5e2”、“-1E-16”都可以表示数值 | 可以将数值字符串看成3部分：整数位`A`、小数位`.B`和指数位`e/EC`。所以我们可以先扫描整数位（包括符号位），判断是否都是`0~9`范围内的字符；接着如果再遇到小数点的话，就扫描小数部分（小数点前面或后面都可以没有数字，但是不能同时都没有数字）；最后，如果遇到`e`或者`E`，则开始扫描指数部分（e或E 的前后必须要同时有数字）。最最后，还要保证此时已经扫描到字符串的末尾了。
284   |  58.1 翻转单词顺序：输入一个英文句子，翻转句子中单词的顺序，但单词内字符的顺序不变。 |  输入字符串 "I am a student."，则输出"student. a am I"。  | 首先翻转整个字符串，然后再一次翻转各个单词
286  |   58.2. 左旋转字符串：把字符串前面的若干个字符转移到字符串的尾部。| 输入字符串"abcdefg"和数字2，该函数将返回左旋转2位得到的结果"cdefgab"  | 跟上一道题类似，先翻转整个字符串，然后再分别翻转后 n 位，以及前面剩下的一部分字符。
318  |  67.把字符串转换成整数：功能类似于 atoi 函数 |  输入 '+123', '  -34', '54.33', '56yy' 这样的字符串，可以输出相应的数值  | 从前往后一次遍历各个字符，在将每一位转成数字后，将上一次计算得到的值乘以 10 加上当前位。需要注意的是符号位、非数字字符、NULL 以及大数越界的问题。
...
209  |  40. 找出数组中最小的k个数   | 例如输入4、5、1、6、2、7、3、8 这 8 个数字，则最小的 4 个数字是1、2、3、4 | 类似于快排，利用 randomPartion 函数来排序，同时基于数组的第 k 个元素来调整，这样就使得比第 k 个数字小的排在前面，大的排在后面，但是不需要整个数组的所有元素都排好序。时间复杂度O(n)
